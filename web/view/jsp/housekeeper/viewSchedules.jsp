...
<%@ page import="model.User, java.util.List, model.HousekeeperSchedule" %>
...
<% 
    ...
    List<HousekeeperSchedule> schedules = (List<HousekeeperSchedule>) request.getAttribute("schedules");
%>
...
<head>
    ...
    <title>Xem Lịch Làm Việc - Giúp Việc 24h</title>
            ...
            <div class="header-content">
                <div class="header-left">
                    ...
                    <div class="header-text">
                        <h1>Lịch Làm Việc</h1>
                        <p>Quản lý và theo dõi lịch trình công việc của bạn</p>
                    </div>
                </div>
                <div class="header-stats">
                    <div class="stat-item">
                        <div class="stat-value">${schedules != null ? schedules.size() : 0}</div>
            ...
            <div class="filter-grid">
                ...
                <div class="form-group">
                    ...
                    <select id="status" name="status" class="form-select">
                        ...
                        <option value="1" ${param.status == '1' ? 'selected' : ''}>Có sẵn</option>
                        <option value="0" ${param.status == '0' ? 'selected' : ''}>Không có sẵn</option>
                    </select>
                </div>
                <div class="form-group">
                    <button class="filter-btn" onclick="filterSchedules()">
        ...
        <div class="table-container">
            <div class="table-header">
                ...
                <div class="table-info">
                    Hiển thị ${schedules != null ? schedules.size() : 0} kết quả
                </div>
            </div>
            ...
            <c:choose>
                <c:when test="${empty schedules}">
                    <div class="empty-state">
                        ...
                        <p>Hiện tại chưa có ca làm việc nào phù hợp với bộ lọc của bạn.</p>
                    </div>
                </c:when>
                <c:otherwise>
                        ...
                        <tbody>
                            <c:forEach var="schedule" items="${schedules}" varStatus="loop">
                                <tr>
                                    ...
                                    <td>
                                        <c:if test="${not empty schedule.startTimeAsDate}">
                                            <div style="display: flex; flex-direction: column; gap: 0.25rem;">
                                                <span style="font-weight: 500; color: var(--text-primary);">
                                                    <fmt:formatDate value="${schedule.startTimeAsDate}" pattern="dd/MM/yyyy" />
                                                </span>
                                                <span style="font-size: 0.75rem; color: var(--text-secondary);">
                                                    <fmt:formatDate value="${schedule.startTimeAsDate}" pattern="HH:mm" />
                                                </span>
                                            </div>
                                        </c:if>
                                        <c:if test="${empty schedule.startTimeAsDate}">
                                            <span style="color: var(--text-secondary);">N/A</span>
                                        </c:if>
                                    </td>
                                    <td>
                                        <c:if test="${not empty schedule.endTimeAsDate}">
                                            <div style="display: flex; flex-direction: column; gap: 0.25rem;">
                                                <span style="font-weight: 500; color: var(--text-primary);">
                                                    <fmt:formatDate value="${schedule.endTimeAsDate}" pattern="dd/MM/yyyy" />
                                                </span>
                                                <span style="font-size: 0.75rem; color: var(--text-secondary);">
                                                    <fmt:formatDate value="${schedule.endTimeAsDate}" pattern="HH:mm" />
                                                </span>
                                            </div>
                                        </c:if>
                                        <c:if test="${empty schedule.endTimeAsDate}">
                                            <span style="color: var(--text-secondary);">N/A</span>
                                        </c:if>
                                    </td>
                                    <td>
                                        <span class="status-badge ${schedule.available ? 'status-available' : 'status-unavailable'}">
